...
mkdir -p "../map_data/landcover"
pushd "../map_data/landcover"


if ! [ -f "ne_10m_antarctic_ice_shelves_polys.zip" ]; then
  curl -fO https://naciscdn.org/naturalearth/10m/physical/ne_10m_antarctic_ice_shelves_polys.zip
fi

if ! [ -f "ne_10m_urban_areas.zip" ]; then
  curl -fO https://naciscdn.org/naturalearth/10m/cultural/ne_10m_urban_areas.zip
fi

if ! [ -f "ne_10m_glaciated_areas.zip" ]; then
  curl -fO https://naciscdn.org/naturalearth/10m/physical/ne_10m_glaciated_areas.zip
fi

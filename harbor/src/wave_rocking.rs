...
pub fn apply_wave_rocking(
    ...
    mut ship_scene_instances: Query<
        (&ShipScenePlacement, &mut Transform),
    ...
    for (physical_ship, appearance, children) in &physical_ships {
        ...
        let roll = physical_ship.roll_amplitude_radians * (roll_phase + physical_ship.roll_phase_offset).sin();
        let pitch = physical_ship.pitch_amplitude_radians * (pitch_phase + physical_ship.pitch_phase_offset).sin();
